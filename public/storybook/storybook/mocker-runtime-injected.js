      ...
      if (seg)
        if (path.length > 0) {
          const pathTrailing = '/' === path[path.length - 1],
            segLeading = '/' === seg[0]
          path +=
            pathTrailing && segLeading ? seg.slice(1) : pathTrailing || segLeading ? seg : `/${seg}`
        } else path += seg
...
var { now } = Date,
  ModuleMocker = class {
    ...
    async invalidate() {
      ...
      ids.length &&
        (await this.rpc.invalidate(ids), await this.interceptor.invalidate(), this.registry.clear())
    }
